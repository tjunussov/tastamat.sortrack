      ...
      b-card.w-100(no-body)
        ...
        b-card-footer
          ...
          b-dropdown.button-block.w-100(
            ...
            :split-variant="weight>0 && weight < 15 && plomba? (selectedBag.batch.length ? 'warning':'primary'):(selectedBag.batch.length?'outline-warning':'outline-primary')" 
            :variant="selectedBag.batch.length?'outline-warning':'outline-primary'" @click="closeBag")
            template(slot="button-content") 
...
export default {
  ...
  methods:{
    ...
    closeBag(){
      ...
      if(this.selectedBag.batch.length){

        this.$formBagByPacklist({
          ppi:this.selected,
          packList:this.selectedBag.batch,
          weight:this.weight,
