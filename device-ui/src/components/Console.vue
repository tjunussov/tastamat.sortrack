...
export default {
  ...
  mounted(){
    ...
    this.$bus.$on('keyboard:keydown:enter:u',this.toggleUser);
  ...
  beforeDestroy(){
    ...
    this.$bus.$off('keyboard:keydown:enter:u',this.toggleUser);
  ...
  methods:{
    searchBarcode(barcode){
      ...
      $device.get('/on?color=1&search=1');

      $http.get(this.barcode).then((resp)=>{
        this.response = resp.data

        if(this.isPolkaSetupEnded && this.response.next.postindex){

          this.toDestination = this.response.next.postindex;
          var p = this.destinations[this.toDestination];

          if(p){
            this.toPolka = this.polkas[p];
            $device.get('/on?color=1&put='+this.toPolka);
          }

        } else {
          alert("Полки не настроены!");
          $device.get('/on?color=3&put=1');
        }
      }).catch(()=>{
        $device.get('/on?color=1&error=1');
      });
    },
    beginSetupPolkas(){
      ...
      if(this.pin != 0 ){
        ...
        this.polkaInterval = window.setInterval(()=>{
          $device.get('/on?color=3&bind='+this.pin);
        },1100);
      }
    },
    togglePolka(polka){
      ...
      if(!this.isPolkaSetupStarted) this.beginSetupPolkas();
        ...
        if(this.polka ){ 
          ...
          this.polkaInterval = window.setInterval(()=>{
            $device.get('/on?color=2&bind='+this.polkas[this.polka]);
          },1100);
        }
      }
    },
